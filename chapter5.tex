...
\textbf{Textbook:} Kurose \& Ross 8e, Figure 5.1 / 5.2 --- ``Per-router control plane'' vs.\ ``SDN control plane''\\
\textbf{Google:} ``per-router control vs SDN control plane diagram comparison''\\
\textit{Side-by-side comparison showing distributed routing algorithms in each router vs.\ a centralized remote controller installing flow tables.}
\end{imagebox}
...
\textbf{Textbook:} Kurose \& Ross 8e, Figure 5.3 --- ``Abstract graph model'' and Figure 5.5 --- ``Dijkstra's algorithm: example''\\
\textbf{Google:} ``Dijkstra shortest path algorithm network routing example''\\
\textit{Weighted graph with nodes u--z and the step-by-step shortest-path tree computed by Dijkstra's algorithm.}
\end{imagebox}
...
\textbf{Textbook:} Kurose \& Ross 8e, Figure 5.6 --- ``Distance-vector (DV) algorithm''\\
\textbf{Google:} ``distance vector routing algorithm Bellman-Ford example diagram''\\
\textit{Iterative exchange of distance vectors between neighboring nodes converging to least-cost paths.}
\end{imagebox}
...
\textbf{Textbook:} Kurose \& Ross 8e, Figure 5.7 --- ``An autonomous system with areas''\\
\textbf{Google:} ``OSPF areas backbone area border router diagram''\\
\textit{Two-level OSPF hierarchy showing backbone (Area 0), local areas, area border routers, and boundary routers.}
\end{imagebox}
...
\textbf{Textbook:} Kurose \& Ross 8e, Figure 5.8 --- ``eBGP and iBGP connections''\\
\textbf{Google:} ``BGP eBGP iBGP autonomous system routing diagram''\\
\textit{Multiple ASes with eBGP sessions between border routers and iBGP sessions distributing routes within each AS.}
\end{imagebox}
...
\textbf{Textbook:} Kurose \& Ross 8e, Figure 5.19 --- ``ICMP message types''\\
\textbf{Google:} ``ICMP message types traceroute ping diagram''\\
\textit{Table of ICMP type/code values and illustration of how ping and traceroute use ICMP messages.}
\end{imagebox}
...
\textbf{Textbook:} Kurose \& Ross 8e, Figure 5.14 --- ``SDN controller and network-control applications''\\
\textbf{Google:} ``SDN architecture controller northbound southbound API diagram''\\
\textit{Three-layer SDN stack: data-plane switches, controller with southbound/northbound APIs, and network-control applications.}
\end{imagebox}
